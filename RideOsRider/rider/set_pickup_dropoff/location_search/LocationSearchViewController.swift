...
public class LocationSearchViewController: UIViewController {
    ...
    public override func viewDidLoad() {
        ...
        locationSearchViewModel.initialState
            .observeOn(schedulerProvider.mainThread())
            .subscribe(onNext: { [unowned self] in self.setupInitialState($0) })
            .disposed(by: disposeBag)
    }
    ...
    private func setupInitialState(_ initialState: LocationSearchInitialState) {
        if initialState.pickupSearchBoxState.location != nil {
            locationSearchView.pickupTextFieldText = initialState.pickupSearchBoxState.location?.displayName
        }
        if initialState.dropoffSearchBoxState.location != nil {
            locationSearchView.dropoffTextFieldText = initialState.dropoffSearchBoxState.location?.displayName
        }
        locationSearchView.isPickupTextFieldEnabled = initialState.pickupSearchBoxState.enabled
        locationSearchView.isDropoffTextFieldEnabled = initialState.dropoffSearchBoxState.enabled
        switch initialState.focus {
        case .pickup:
            locationSearchView.pickupTextFieldClearsOnBeginEditing = false
            locationSearchView.makePickupTextFieldFirstResponder()
        default:
            locationSearchView.dropoffTextFieldClearsOnBeginEditing = false
