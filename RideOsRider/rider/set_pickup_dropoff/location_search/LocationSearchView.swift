...
class LocationSearchView: UIView {
    ...
    public var pickupTextFieldTextEvents: Observable<String> {
        // NOTE: subscribe to editingChanged control events instead of using pickupTextField.rx.text directly so that
        // we don't trigger events when the text is changed programmatically
        return pickupTextField.rx
            .controlEvent(.editingChanged)
            .map { [pickupTextField] _ in pickupTextField.text ?? "" }
    }

    public var dropoffTextFieldTextEvents: Observable<String> {
        // NOTE: subscribe to editingChanged control events instead of using dropoffTextField.rx.text directly so that
        // we don't trigger events when the text is changed programmatically
        return dropoffTextField.rx
            .controlEvent(.editingChanged)
            .map { [dropoffTextField] _ in dropoffTextField.text ?? "" }
    }
    ...
    public var isPickupTextFieldEnabled: Bool {
        get {
            return !pickupTextField.isEnabled
        }
        set {
            pickupTextField.isEnabled = newValue
        }
    }

    public var isDropoffTextFieldEnabled: Bool {
        get {
            return !dropoffTextField.isEnabled
        }
        set {
            dropoffTextField.isEnabled = newValue
        }
    }

    public func makePickupTextFieldFirstResponder() {
    ...
    public var pickupTextFieldClearsOnBeginEditing: Bool {
        get {
            return pickupTextField.clearsOnBeginEditing
        }
        set {
            pickupTextField.clearsOnBeginEditing = newValue
        }
    }

    public var dropoffTextFieldClearsOnBeginEditing: Bool {
        get {
            return dropoffTextField.clearsOnBeginEditing
        }
        set {
            dropoffTextField.clearsOnBeginEditing = newValue
        }
    }
    ...
    override init(frame: CGRect) {
        ...
        doneButton.setTitleColor(
            RideOsRiderResourceLoader.instance.getColor("ai.rideos.rider.location-search.done-button.title-color"),
            for: .normal
        )
